...
QWORD TickGetFullScale(void)
{
	QWORD qw;

	GetTickCopy();

	((BYTE*)&qw)[0] = vTickReading[0];	// Note: This copy must be done one
	((BYTE*)&qw)[1] = vTickReading[1];	// byte at a time to prevent misaligned
	((BYTE*)&qw)[2] = vTickReading[2];	// memory reads, which will reset the PIC.
	((BYTE*)&qw)[3] = vTickReading[3];
	((BYTE*)&qw)[4] = vTickReading[4];
	((BYTE*)&qw)[5] = vTickReading[5];
